...
export default function InvestmentChat({ riskTolerance, investmentReason, apiBase, market = "Global" }: InvestmentChatProps) {
...
Provide personalized investment advice based on their profile. Be professional, helpful, and clear. Always consider their risk tolerance and investment goals when making recommendations. Discuss diversification, asset allocation, and risk management principles.`
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      const devProxy = (import.meta as any).env?.DEV ? "/advisor-api" : null;
      const envUrl = (import.meta as any).env?.VITE_ADVISOR_URL || undefined;
      const base = apiBase || (devProxy ?? envUrl ?? "http://localhost:3000");
  ...
  return (
    <div className="flex-1 flex flex-col bg-background" style={{ width: "100%", height: "100%" }}>
      <div className="flex-1 overflow-y-auto p-4 space-y-4" style={{ maxHeight: 440 }}>
        {messages.length === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center space-y-4">
              <div className="text-4xl">📊</div>
              <h2 className="text-lg font-bold text-foreground">Welcome to Your Investment Advisor</h2>
          ...
          <>
            {messages.map((message) => {
              ...
              const display = isLong && !isExpanded ? message.content.slice(0, 400) + "..." : message.content

              return (
                <div key={message.id} className={`flex ${message.role === "user" ? "justify-end" : "justify-start"}`}>
                  <Card
                    className={`max-w-md px-4 py-3 ${
                      message.role === "user" ? "" : "bg-card text-foreground border-border"
                    }`}
                    style={
                      message.role === "user"
                    ...
                    {isLong && (
                      <div className="mt-2 text-right">
                        <button
                          ...
                          style={{ color: message.role === "user" ? "#e6fffa" : "#047857" }}
                          onClick={() => {
                ...
                <Card className="bg-card px-4 py-3 border-border">
                  <div style={{ width: 16, height: 16 }}>
                    <svg width="16" height="16" viewBox="0 0 50 50">
                      <circle cx="25" cy="25" r="20" fill="none" stroke="#059669" strokeWidth="4" strokeDasharray="31.4 31.4">
      ...
      <div style={{ borderTop: "1px solid #e6e6e6", padding: 12, background: "#fff" }}>
